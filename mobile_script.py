...
class Main:
    ...
    def solve_captcha(self, sessid):
        try:
           ...
           json_data =  {
            ...
            print(f"Error: {e}")
            print("Captcha Không hợp lệ | https://discord.gg/tMqvzJkG4C Vào để được support")
            self.solve_captcha(sessid)
    ...
    def views_loop(self, sessid, alpha_key):
        while True:
            ...
            if "Dịch vụ này hiện không hoạt động" in decryped_answer:
                print("Hiện không có lượt xem")
                input()
                sys.exit()

            elif "Hệ thống quá tải" in decryped_answer:
                print("Máy chủ bận ! (waiting 10s)")
                time.sleep(10)
                continue
            ...
            if latency > 3:
                print(f"Gửi view video [id={VIDEO}]")

            decrypted_response = self.decrypt(send_views.text)

            if "Quá nhiều yêu cầu. Vui long chậm lại." in decrypted_response:
                print("Ratelimited")
                time.sleep(120)
                continue
            ...
            print(f"Thời Gian: {timer} (not animated")
            time.sleep(timer)
            
            print(f"Gửi View Thành Công ...")
